...
@export var footstep_pitch_min: float = 0.95
@export var footstep_pitch_max: float = 1.05
...
@onready var footstep_audio_player: AudioStreamPlayer3D = $FootstepAudio
...
func _try_play_footstep(delta: float) -> void:
	if not is_on_floor():
		return
	...
	_footstep_timer = footstep_interval_walk
	
	var index: int = randi_range(0, footstep_sfx.size() - 1)
	var stream: AudioStream = footstep_sfx[index]
	var pitch: float = randf_range(footstep_pitch_min, footstep_pitch_max)
	
	footstep_audio_player.stream = stream
	footstep_audio_player.volume_db = footstep_volume_db
	footstep_audio_player.pitch_scale = pitch
